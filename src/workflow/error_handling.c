...
WorkflowErrorType workflow_handle_network_error(Workflow* wf, const char* error_msg) {
    ...
    if (error_msg) {
        ...
        wf->error_message =
            workflow_strdup("Network error: Unable to connect to required services");
    }
 ...
 * @return WorkflowErrorType
 */
WorkflowErrorType workflow_handle_file_io_error(Workflow* wf, const char* filepath,
    ...
    if (filepath && operation) {
        snprintf(error_msg, sizeof(error_msg),
                 "File I/O error: Cannot %s file '%s' (errno: %d, %s)", operation, filepath, errno,
                 strerror(errno));
    } else {
...
WorkflowErrorType workflow_handle_credit_exhausted(Workflow* wf) {
    ...
    wf->error_message = workflow_strdup(
        "Credit exhausted: API budget limit reached. Cannot continue workflow execution.");
    wf->status = WORKFLOW_STATUS_FAILED;
...
WorkflowErrorType workflow_handle_llm_down(Workflow* wf, ProviderType provider_type) {
    ...
    char error_msg[256];
    snprintf(error_msg, sizeof(error_msg),
             "LLM service down: Provider %d is unavailable. Workflow execution paused.",
             (int)provider_type);
 ...
 * @return WorkflowErrorType
 */
WorkflowErrorType workflow_handle_tool_error(Workflow* wf, const char* tool_name,
    ...
    if (tool_name && error_msg) {
        snprintf(full_error, sizeof(full_error),
                 "Tool execution failed: Tool '%s' returned error: %s", tool_name, error_msg);
    } else if (tool_name) {
        snprintf(full_error, sizeof(full_error), "Tool execution failed: Tool '%s' failed",
                 tool_name);
    } else {
        snprintf(full_error, sizeof(full_error), "Tool execution failed: %s",
 ...
 */
bool workflow_handle_error(Workflow* wf, WorkflowNode* node, WorkflowErrorType error_type,
                           const char* error_msg) {
    if (!wf || !node) {
        return false;
    }
