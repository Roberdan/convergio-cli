...
// Simple condition evaluator (supports: key == value, key != value, key < value, key > value, key
// <= value, key >= value) Full implementation would use proper expression parser
static bool evaluate_simple_condition(const char* expr, const WorkflowState* state) {
    if (!expr || !state) {
        return false;
    }

    // Parse: "key == value", "key != value", "key < value", "key > value", "key <= value", "key >=
    // value"
...
bool router_evaluate_condition(const char* condition_expr, const WorkflowState* state) {
...
WorkflowNode* router_get_next_node(Workflow* wf, WorkflowNode* current,
