...
Workflow* pattern_create_review_refine_loop(SemanticID generator_id, SemanticID critic_id,
                                            SemanticID refiner_id, int max_iterations) {
    if (max_iterations <= 0) {
        max_iterations = 3;
    }
    ...
    // Create workflow
    Workflow* wf =
        workflow_create("Review-Refine Loop",
                        "Iterative refinement pattern with generator, critic, and refiner", NULL);
...
Workflow* pattern_create_parallel_analysis(SemanticID* analyst_ids, size_t analyst_count,
                                           SemanticID converger_id) {
    if (!analyst_ids || analyst_count == 0) {
        return NULL;
    }

    Workflow* wf = workflow_create("Parallel Analysis",
                                   "Multiple analysts work in parallel, then converge", NULL);
...
Workflow* pattern_create_sequential_planning(SemanticID* planner_ids, size_t planner_count) {
    if (!planner_ids || planner_count == 0) {
        return NULL;
    }

    Workflow* wf =
        workflow_create("Sequential Planning", "Chain of planners building on each other", NULL);
...
Workflow* pattern_create_consensus_building(SemanticID* participant_ids, size_t participant_count,
    ...
    Workflow* wf =
        workflow_create("Consensus Building", "Multi-agent discussion to reach consensus", NULL);
