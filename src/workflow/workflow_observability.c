 ...
 */
void workflow_log_event(LogLevel level, const char* event, const char* workflow_name, uint64_t workflow_id, const char* details) {
    if (workflow_name) {
        if (details) {
            nous_log(level, LOG_CAT_WORKFLOW, "[workflow:%s id:%llu] %s: %s",
                     workflow_name, (unsigned long long)workflow_id, event, details);
        } else {
            nous_log(level, LOG_CAT_WORKFLOW, "[workflow:%s id:%llu] %s",
                     workflow_name, (unsigned long long)workflow_id, event);
        }
    } else {
        if (details) {
 ...
 */
void workflow_log_node_execution(const Workflow* wf, const WorkflowNode* node, const char* status, const char* details) {
    if (!wf || !node) {
        return;
    }
    ...
    if (details) {
        LOG_INFO(LOG_CAT_WORKFLOW, "[workflow:%s id:%llu] node:%s type:%d status:%s details:%s",
                 workflow_name, (unsigned long long)wf->workflow_id, node_name, (int)node->type, status, details);
    } else {
        LOG_INFO(LOG_CAT_WORKFLOW, "[workflow:%s id:%llu] node:%s type:%d status:%s",
                 workflow_name, (unsigned long long)wf->workflow_id, node_name, (int)node->type, status);
    }
}
...
void workflow_log_error(const Workflow* wf, const char* error_type, const char* error_message) {
    ...
    if (error_message) {
        LOG_ERROR(LOG_CAT_WORKFLOW, "[workflow:%s id:%llu] error_type:%s error:%s",
                  workflow_name, (unsigned long long)wf->workflow_id, error_type, error_message);
    } else {
        LOG_ERROR(LOG_CAT_WORKFLOW, "[workflow:%s id:%llu] error_type:%s",
                  workflow_name, (unsigned long long)wf->workflow_id, error_type);
    }
}
 ...
 */
void workflow_telemetry_node(const Workflow* wf, const WorkflowNode* node, bool success, double latency_ms) {
    ...
    char event_type[128];
    snprintf(event_type, sizeof(event_type), "workflow_node_%s_%s",
             success ? "success" : "failure", node->name ? node->name : "unnamed");
...
void workflow_telemetry_error(const Workflow* wf, const char* error_type) {
    ...
    char event_type[128];
    snprintf(event_type, sizeof(event_type), "workflow_error_%s", error_type ? error_type : "unknown");
    telemetry_record_error(event_type);
...
void workflow_audit_log(const Workflow* wf, const char* operation, const char* details) {
    ...
    if (details) {
        ...
        LOG_INFO(LOG_CAT_WORKFLOW, "[AUDIT workflow:%s id:%llu] operation:%s",
                 workflow_name, (unsigned long long)wf->workflow_id, operation);
    }
...
void workflow_security_log(const Workflow* wf, const char* security_event, const char* details) {
    ...
    if (details) {
        ...
        LOG_WARN(LOG_CAT_WORKFLOW, "[SECURITY workflow:%s id:%llu] event:%s",
                 workflow_name, (unsigned long long)wf->workflow_id, security_event);
    }
