...
int workflow_node_add_edge(WorkflowNode* from, WorkflowNode* to, const char* condition) {
    ...
    if (from->next_node_count >= from->next_node_capacity) {
        size_t new_capacity =
            from->next_node_capacity == 0 ? INITIAL_NODE_CAPACITY : from->next_node_capacity * 2;
        if (new_capacity > MAX_EDGES_PER_NODE) {
            new_capacity = MAX_EDGES_PER_NODE;
        }
        WorkflowNode** new_nodes = realloc(from->next_nodes, new_capacity * sizeof(WorkflowNode*));
        if (!new_nodes) {
            return -1;
        }
