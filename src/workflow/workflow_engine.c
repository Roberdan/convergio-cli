...
extern void workflow_log_event(LogLevel level, const char* event, const char* workflow_name,
                               uint64_t workflow_id, const char* details);
extern void workflow_log_node_execution(const Workflow* wf, const WorkflowNode* node,
                                        const char* status, const char* details);
extern void workflow_log_error(const Workflow* wf, const char* error_type,
...
extern void workflow_telemetry_node(const Workflow* wf, const WorkflowNode* node, bool success,
                                    double latency_ms);
...
extern void workflow_security_log(const Workflow* wf, const char* security_event,
                                  const char* details);
...
static int execute_action_node(Workflow* wf, WorkflowNode* node, const char* input, char** output) {
    ...
    if (!workflow_check_network(5)) {
        workflow_handle_network_error(wf,
                                      "Network unavailable: Cannot connect to required services");
        free(effective_prompt);
        return -1;
    }
    ...
    TokenUsage usage = {0};
    char* response = llm_chat_with_model("claude-sonnet-4-20250514", agent->system_prompt,
                                         effective_prompt, &usage);
    ...
    if (!response) {
        ...
        workflow_handle_error(wf, node, WORKFLOW_ERROR_UNKNOWN,
                              "Agent execution failed - check network, API key, and credit");
        return -1;
    }
...
static void handle_node_failure(Workflow* wf, WorkflowNode* node, char* current_input,
                                char* node_output, double latency_ms) {
    ...
    workflow_log_event(LOG_LEVEL_ERROR, "workflow_execute_failed", wf->name, wf->workflow_id,
                       wf->error_message);
}
...
int workflow_execute(Workflow* wf, const char* input, char** output) {
    ...
    workflow_log_event(LOG_LEVEL_INFO, "workflow_execute_completed", wf->name, wf->workflow_id,
                       NULL);
...
int workflow_cancel(Workflow* wf) {
    ...
    if (wf->status == WORKFLOW_STATUS_COMPLETED || wf->status == WORKFLOW_STATUS_CANCELLED) {
        return -1;
    }
