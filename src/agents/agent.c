...
NousAgent* nous_create_agent(const char* name, const char* essence) {
    ...
    dispatch_queue_attr_t work_attr = dispatch_queue_attr_make_with_qos_class(
...
static void transition_state(NousAgent* agent, AgentState new_state) {
    ...
    #ifdef DEBUG
    os_log_info(OS_LOG_DEFAULT, "Agent '%s' transition: %s -> %s",
                agent->name, state_names[old_state], state_names[new_state]);
    #endif
...
int nous_agent_listen(NousAgent* agent, SemanticID space) {
    ...
    agent->memories = realloc(agent->memories,
                              (agent->memory_count + 1) * sizeof(SemanticID));
    if (agent->memories) {
...
int nous_agent_think(NousAgent* agent, ParsedIntent* intent,
                     ThinkingCallback callback, void* ctx) {
    if (!agent || !intent) return -1;
...
int nous_agent_trust(NousAgent* agent, SemanticID human, float trust_level) {
    if (!agent || trust_level < 0.0f || trust_level > 1.0f) return -1;
    ...
    SemanticID* new_humans = realloc(agent->trusted_humans,
                                     (agent->trust_count + 1) * sizeof(SemanticID));
    float* new_levels = realloc(agent->trust_levels,
                                (agent->trust_count + 1) * sizeof(float));
...
int nous_agent_add_skill(NousAgent* agent, const char* skill) {
    if (!agent || !skill) return -1;

    char** new_skills = realloc(agent->skills,
                                (agent->skill_count + 1) * sizeof(char*));
    if (!new_skills) return -1;

    agent->skills = new_skills;
    agent->skills[agent->skill_count] = strdup(skill);

    if (!agent->skills[agent->skill_count]) return -1;
