...
static const char* ALI_SYSTEM_PROMPT_TEMPLATE =
    ...
    "1. **Use glob/grep** instead of shell find/grep commands when possible\n"
    "2. **Use edit** for file modifications (more precise than file_write, creates backups)\n"
    "3. **Use file_delete** instead of shell rm (files go to Trash safely)\n"
    "4. Backups are stored in `~/.convergio/backups/` and Trash in `~/.convergio/trash/`\n\n"
    "### Memory Tools\n"
    ...
    "- Delete files → MUST call `file_delete` (prefer over shell rm - uses Trash)\n"
    ...
    "- NEVER claim files were found without calling `glob` or `file_list`\n"
    "- NEVER claim files were deleted without calling `file_delete`\n"
