...
static const MaestroPersonality MAESTRO_PERSONALITIES[] = {
    ...
    {"cicerone-civica", "Cicerone",
     "Anche nel Senato romano le cose non sempre andavano lisce.",
    ...
    {"ippocrate-corpo", "Ippocrate",
     "Come in medicina, a volte serve una seconda diagnosi.",
    ...
    {"ali-principal", "Ali",
     "Mi dispiace, qualcosa non ha funzionato come previsto.",
...
static const ErrorPattern ERROR_PATTERNS[] = {
    ...
    {"exceeded maximum iterations",
     "Ho fatto molti tentativi ma mi sono bloccato.",
    ...
    {"Memory allocation failed",
     "Il sistema è un po' affaticato.",
     "Prova a chiudere altre applicazioni e riprova."},

    {"Tool execution failed",
     "Non sono riuscito a usare uno strumento.",
    ...
    {"Provider not configured",
     "Il sistema non è ancora configurato completamente.",
     "Chiedi a un adulto di eseguire /setup per configurare tutto."},

    {"parse tool call",
     "Ho avuto difficoltà a capire come procedere.",
...
char* education_interpret_error(const char* error_msg, const char* agent_id) {
    ...
    if (!friendly) return strdup(error_msg);

    if (personality && pattern) {
        snprintf(friendly, msg_size,
            "%s: %s\n\n%s\n\n💡 %s",
            personality->name,
            personality->apology_style,
            pattern->friendly_message,
            pattern->suggestion);
    } else if (personality) {
        snprintf(friendly, msg_size,
            "%s: %s\n\n%s",
...
bool education_should_interpret_error(const char* error_msg) {
    ...
    if (edition_current() != EDITION_EDUCATION) return false;

    // Check if it starts with "Error:" or contains known patterns
    if (strncmp(error_msg, "Error:", 6) == 0) return true;
