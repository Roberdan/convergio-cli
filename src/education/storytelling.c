...
static const char* FILLER_WORDS_IT[] = {"ehm",      "uhm",          "ah",
                                        "eh",       "oh",           "mah",
                                        "cioe",     "praticamente", "fondamentalmente",
                                        "insomma",  "tipo",         "diciamo",
                                        "ecco",     "allora",       "quindi",
                                        "comunque", "niente",       "vabbene",
                                        "capito",   "no?",          NULL};

// English filler words
static const char* FILLER_WORDS_EN[] = {
    "um",       "uh",        "ah",       "er",       "eh",     "like",    "basically",
    "actually", "literally", "honestly", "you know", "I mean", "sort of", "kind of",
    "right",    "so",        "well",     "anyway",   "okay",   "yeah",    NULL};
...
static const char* SOCRATES_HOOKS[] = {
    ...
    "Do you know why Socrates was sentenced to death?", NULL};

static const char* EUCLID_HOOKS[] = {"A king once asked: 'Is there an easy path to geometry?'",
                                     "Think of a world without numbers. Impossible, right?",
                                     "It all began with a point, a line, and a question...", NULL};
...
static const char* FEYNMAN_HOOKS[] = {
    ...
    "The secret of the universe is hidden in a cup of coffee...", NULL};
...
static const char* HERODOTUS_HOOKS[] = {
    ...
    "Have you ever wondered why we speak of a 'Pyrrhic victory'?", NULL};

static const char* DARWIN_HOOKS[] = {"Sailing to remote islands, I noticed something strange...",
...
static const char* LEONARDO_HOOKS[] = {
    ...
    "A bird taught me to fly. In theory.", "Before Instagram, there was my sketchbook...", NULL};
...
static const char* MANZONI_HOOKS[] = {
    ...
    "A broken promise changed the history of Italy.", NULL};

static const char* SHAKESPEARE_HOOKS[] = {
    "To be or not to be - but what does it really mean?", "A prince, a ghost, and a revenge...",
    "All the world's a stage. But what role do you play?", NULL};

static const char* MOZART_HOOKS[] = {"I was 5 years old when I wrote my first symphony...",
                                     "Music is not in the notes, but in the silence between them.",
                                     "Hear this melody? I dreamed it last night.", NULL};

static const char* CICERO_HOOKS[] = {"Citizens of Rome! How long will you abuse our patience?",
                                     "Three words can change the world: Veni, Vidi, Vici.",
                                     "The art of speech is the most powerful weapon.", NULL};

static const char* SMITH_HOOKS[] = {"Why is a pin-maker the secret to wealth?",
...
static const char* LOVELACE_HOOKS[] = {
    "My father was a poet. I preferred machines.", "Imagine a loom that thinks...",
    "Before the word 'computer' existed, I dreamed of it.", NULL};
...
static const char* HIPPOCRATES_HOOKS[] = {
    ...
    "2500 years ago, medicine was magic. Then everything changed.", NULL};
...
static const char* HUMBOLDT_HOOKS[] = {
    ...
    "The Amazon taught me more than any library ever could.", NULL};
...
static const char* CHRIS_HOOKS[] = {
    ...
    "What if you could change someone's life in 18 minutes?", NULL};

static const MaestroHooks MAESTRO_HOOKS[] = {
    {"socrate-filosofia", SOCRATES_HOOKS},  {"euclide-matematica", EUCLID_HOOKS},
    {"feynman-fisica", FEYNMAN_HOOKS},      {"erodoto-storia", HERODOTUS_HOOKS},
    {"darwin-scienze", DARWIN_HOOKS},       {"leonardo-arte", LEONARDO_HOOKS},
    {"manzoni-italiano", MANZONI_HOOKS},    {"shakespeare-inglese", SHAKESPEARE_HOOKS},
    {"mozart-musica", MOZART_HOOKS},        {"cicerone-civica", CICERO_HOOKS},
    {"smith-economia", SMITH_HOOKS},        {"lovelace-informatica", LOVELACE_HOOKS},
    {"ippocrate-corpo", HIPPOCRATES_HOOKS}, {"humboldt-geografia", HUMBOLDT_HOOKS},
    {"chris-storytelling", CHRIS_HOOKS},    {NULL, NULL}};
...
FillerAnalysis* storytelling_analyze_fillers(const char* text, const char* language) {
    ...
    const char** filler_list =
        (language && strcmp(language, "en") == 0) ? FILLER_WORDS_EN : FILLER_WORDS_IT;
...
TEDTimerState storytelling_ted_timer_state(void) {
    ...
    if (state.elapsed_seconds > TED_TOTAL_TIME) {
        ...
        state.section_remaining =
            TED_SECTION_HOOK + TED_SECTION_MAIN + TED_SECTION_PILLARS - state.elapsed_seconds;
    } else {
...
const char* storytelling_section_label(const char* section) {
    ...
    if (strcmp(section, "hook") == 0)
        return "HOOK - Capture attention";
    if (strcmp(section, "main_idea") == 0)
        return "MAIN IDEA - Your throughline";
    if (strcmp(section, "three_pillars") == 0)
        return "THREE PILLARS - Your key points";
    if (strcmp(section, "call_to_action") == 0)
        return "CALL TO ACTION - Memorable close";
    if (strcmp(section, "overtime") == 0)
        return "TIME'S UP!";
    if (strcmp(section, "not_started") == 0)
...
float storytelling_evaluate_hail(const char* text, float scores[4]) {
    ...
    int personal = count_word_occurrences(text, "I") + count_word_occurrences(text, "my") +
                   count_word_occurrences(text, "me") + count_word_occurrences(text, "myself");
    ...
    int decisive =
        count_word_occurrences(text, "believe") + count_word_occurrences(text, "I firmly believe") +
        count_word_occurrences(text, "it's important") + count_word_occurrences(text, "must");
    scores[2] = 0.5f + (float)decisive * 0.1f;
    if (scores[2] > 1.0f)
        scores[2] = 1.0f;

    // L - Love: Check for audience engagement
    int engagement = count_word_occurrences(text, "you") + count_word_occurrences(text, "your") +
                     count_word_occurrences(text, "together") + count_word_occurrences(text, "we") +
...
int storytelling_cnepr_duration(int section) {
    switch (section) {
    case 0:
        return 120; // Connection: 2 min
    case 1:
        return 300; // Narration: 5 min
    case 2:
        return 360; // Explanation: 6 min
    case 3:
        return 180; // Persuasion: 3 min
    case 4:
        return 120; // Revelation: 2 min
    default:
        return 0;
    }
}
