...
static const char* reminder_type_to_string(AnnaReminderType type) {
    ...
    case ANNA_REMINDER_BREAK:
        return "break";
    case ANNA_REMINDER_CELEBRATION:
        return "celebration";
...
static AnnaReminderType string_to_reminder_type(const char* str) {
    ...
    if (strcmp(str, "homework") == 0)
        return ANNA_REMINDER_HOMEWORK;
    if (strcmp(str, "spaced_repetition") == 0)
        return ANNA_REMINDER_SPACED_REPETITION;
    if (strcmp(str, "break") == 0)
        return ANNA_REMINDER_BREAK;
    if (strcmp(str, "celebration") == 0)
        return ANNA_REMINDER_CELEBRATION;
    if (strcmp(str, "session") == 0)
        return ANNA_REMINDER_SESSION;
    if (strcmp(str, "goal") == 0)
        return ANNA_REMINDER_GOAL;
    return ANNA_REMINDER_HOMEWORK;
}
 ...
 */
static int64_t anna_store_reminder(int64_t student_id, AnnaReminderType type, const char* content,
                                   time_t scheduled_at) {
    if (!g_anna_edu_initialized || !content)
        return -1;
    ...
    snprintf(formatted_content, sizeof(formatted_content), "REMINDER|%s|%ld|%s",
             reminder_type_to_string(type), (long)scheduled_at, content);

    const char* sql = "INSERT INTO inbox (student_id, content, source, processed, created_at) "
...
int64_t anna_homework_reminder(int64_t student_id, const char* subject, const char* assignment,
                               time_t due_date) {
    if (!g_anna_edu_initialized || !subject || !assignment)
        return -1;

    // Create reminder content
    char content[ANNA_EDU_MAX_CONTENT];
    snprintf(content, sizeof(content), "Homework due in %s: %s", subject, assignment);

    // Schedule 24h before due date
    time_t remind_at_1 = due_date - ANNA_EDU_HOMEWORK_ADVANCE_1;
    int64_t reminder_id_1 =
        anna_store_reminder(student_id, ANNA_REMINDER_HOMEWORK, content, remind_at_1);

    // Schedule 1h before due date
    time_t remind_at_2 = due_date - ANNA_EDU_HOMEWORK_ADVANCE_2;
    int64_t reminder_id_2 =
        anna_store_reminder(student_id, ANNA_REMINDER_HOMEWORK, content, remind_at_2);

    fprintf(stderr, "[anna_edu] Scheduled homework reminders for %s (IDs: %lld, %lld)\n", subject,
            (long long)reminder_id_1, (long long)reminder_id_2);

    return reminder_id_1; // Return first reminder ID
}

int64_t anna_spaced_repetition_reminder(int64_t student_id, const char* topic, time_t next_review) {
    if (!g_anna_edu_initialized || !topic)
        return -1;

    char content[ANNA_EDU_MAX_CONTENT];
    snprintf(content, sizeof(content), "Time to review: %s", topic);

    int64_t reminder_id =
        anna_store_reminder(student_id, ANNA_REMINDER_SPACED_REPETITION, content, next_review);
...
int64_t anna_adhd_break_reminder(int64_t student_id, int64_t session_id) {
    ...
    int64_t reminder_id = anna_store_reminder(student_id, ANNA_REMINDER_BREAK, content, break_time);
...
int anna_celebration_notify(int64_t student_id, const AnnaCelebration* celebration) {
    if (!g_anna_edu_initialized || !celebration)
        return -1;
    ...
    snprintf(title, sizeof(title), "%s %s", celebration->emoji ? celebration->emoji : "🎉",
    ...
    if (result == 0) {
        fprintf(stderr, "[anna_edu] Celebration sent for student %lld: %s\n", (long long)student_id,
                celebration->achievement_type);
    }

    return result;
}

int64_t anna_session_reminder(int64_t student_id, const char* subject, time_t scheduled_time) {
    if (!g_anna_edu_initialized || !subject)
        return -1;

    char content[ANNA_EDU_MAX_CONTENT];
    snprintf(content, sizeof(content), "Time to study %s!", subject);

    int64_t reminder_id =
        anna_store_reminder(student_id, ANNA_REMINDER_SESSION, content, scheduled_time);
...
int anna_goal_reminder(int64_t student_id, int64_t goal_id, const char* goal_description,
                       time_t deadline) {
    if (!g_anna_edu_initialized || !goal_description)
        return -1;
    ...
    if (remind_7d > time(NULL)) {
        char content[ANNA_EDU_MAX_CONTENT];
        snprintf(content, sizeof(content), "Goal reminder: %s (7 days left)", goal_description);
    ...
    if (remind_3d > time(NULL)) {
        char content[ANNA_EDU_MAX_CONTENT];
        snprintf(content, sizeof(content), "Goal reminder: %s (3 days left)", goal_description);
    ...
    if (remind_1d > time(NULL)) {
        char content[ANNA_EDU_MAX_CONTENT];
        snprintf(content, sizeof(content), "Goal reminder: %s (1 day left!)", goal_description);
...
int anna_check_due_reminders(void) {
    ...
    const char* sql = "SELECT id, student_id, content "
                      "FROM inbox "
...
int anna_snooze_reminder(int64_t reminder_id, int snooze_minutes) {
    if (!g_anna_edu_initialized || snooze_minutes <= 0)
        return -1;
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (sscanf(old_content, "REMINDER|%31[^|]|%ld|%1023[^\n]", type_str,
                   (long*)&old_scheduled_at, message) == 3) {
            time_t new_scheduled_at = time(NULL) + (snooze_minutes * 60);

            // Update content with new time
            snprintf(content, sizeof(content), "REMINDER|%s|%ld|%s", type_str,
                     (long)new_scheduled_at, message);
...
AnnaReminder** anna_list_reminders(int64_t student_id, int* count) {
    if (!g_anna_edu_initialized || !count)
        return NULL;
    ...
    *count = 0;
    ...
    const char* sql = "SELECT id, student_id, content, created_at "
                      "FROM inbox "
...
bool anna_needs_break(int64_t student_id, int64_t session_id) {
    ...
    time_t reference_time =
        session->current_pomodoro_start > 0 ? session->current_pomodoro_start : session->started_at;
...
int anna_send_accessible_notification(int64_t student_id, const char* title, const char* body) {
    if (!title || !body)
        return -1;
    ...
    if (accessibility && accessibility->tts_enabled) {
        char say_cmd[2048];
        snprintf(say_cmd, sizeof(say_cmd), "say -r %.0f \"%s. %s\"",
                 accessibility->tts_speed * 100.0f, title, body);
        system(say_cmd);
    }
...
AnnaReminderStats anna_get_stats(int64_t student_id) {
    ...
    const char* sql = "SELECT COUNT(*) FROM inbox "
                      "WHERE student_id = ? AND source = 'reminder' AND processed = 0";
