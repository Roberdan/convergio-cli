...
ProgressDashboard* progress_dashboard(int64_t student_id) {
    ...
    // Average quiz score
    const char* sql_quiz =
        "SELECT AVG(score_percent) FROM quiz_history WHERE student_id = ?";
    ...
    if (sqlite3_prepare_v2(g_edu_db, sql_subjects, -1, &stmt, NULL) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            snprintf(entry, sizeof(entry),
                "%s    {\"subject\": \"%s\", \"topics\": %d, \"skill\": %.2f, \"time_hours\": %.1f}",
                first ? "" : ",\n",
                subject,
                topic_count,
                avg_skill,
    ...
    const char* sql_radar =
        "SELECT subject, AVG(skill_level) as avg_skill "
        "FROM learning_progress "
        "WHERE student_id = ? "
        "GROUP BY subject "
        "LIMIT 8";
...
TopicProgress** progress_track_topics(int64_t student_id, int* count) {
    ...
    *count = 0;
    ...
    const char* sql =
        "SELECT subject, topic, skill_level, total_time_spent, "
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (subject) strncpy(tp->subject, subject, sizeof(tp->subject) - 1);
        if (topic) strncpy(tp->topic, topic, sizeof(tp->topic) - 1);
...
char* progress_suggest_next(int64_t student_id) {
    ...
    for (int i = 0; i < topic_count && i < 20; i++) {
        ...
        switch (topics[i]->status) {
            case TOPIC_MASTERED: status_str = "Mastered"; break;
            case TOPIC_COMPLETED: status_str = "Completed"; break;
            case TOPIC_IN_PROGRESS: status_str = "In Progress"; break;
            default: status_str = "Not Started"; break;
        }

        snprintf(line, sizeof(line),
            "- %s: %s (skill: %.0f%%, time: %dmin)\n",
            topics[i]->topic,
            status_str,
            topics[i]->skill_level * 100.0f,
            topics[i]->time_spent_minutes
        );
...
char* progress_parent_report(int64_t student_id, const char* format) {
    ...
    if (strcmp(format, "email") == 0) {
        snprintf(report, 8192,
            ...
            dashboard->topics_completed,
            dashboard->topics_in_progress,
            dashboard->avg_quiz_score,
