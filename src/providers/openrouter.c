/**
 * CONVERGIO OPENROUTER PROVIDER ADAPTER
 *
 * Implements the Provider interface for OpenRouter
 * OpenRouter provides access to 300+ models via OpenAI-compatible API
 * Including DeepSeek, Mistral, Llama, Qwen, and more
 *
...
static bool openrouter_validate_key(Provider* self);
static char* openrouter_chat(Provider* self, const char* model, const char* system,
                             const char* user, TokenUsage* usage);
static char* openrouter_chat_with_tools(Provider* self, const char* model, const char* system,
...
static ProviderError openrouter_init(Provider* self) {
    ...
    if (!api_key || strlen(api_key) == 0) {
        ...
        data->last_error.message = strdup("OPENROUTER_API_KEY not set");
        pthread_mutex_unlock(&data->mutex);
    ...
    LOG_INFO(LOG_CAT_API, "OpenRouter provider initialized");
    return PROVIDER_OK;
}
...
static void openrouter_shutdown(Provider* self) {
    ...
    LOG_INFO(LOG_CAT_API, "OpenRouter provider shutdown");
}
...
static char* openrouter_chat(Provider* self, const char* model, const char* system,
    ...
    // OpenRouter model IDs include provider prefix (e.g., "anthropic/claude-3-opus")
    const char* api_model = model ? model : "openai/gpt-4o";
    ...
    // Build headers - OpenRouter requires HTTP-Referer
    ...
    headers = curl_slist_append(headers, "HTTP-Referer: " OPENROUTER_REFERER);
    headers = curl_slist_append(headers, "X-Title: " OPENROUTER_TITLE);
    ...
    LOG_DEBUG(LOG_CAT_API, "OpenRouter API call: model=%s", api_model);
    ...
    if (res == CURLE_ABORTED_BY_CALLBACK) {
        ...
        LOG_WARN(LOG_CAT_API, "OpenRouter API error: HTTP %ld", http_code);
    } else {
...
static char* openrouter_chat_with_tools(Provider* self, const char* model, const char* system,
    ...
    // Build tools JSON (OpenAI format - OpenRouter is compatible)
    ...
    // Parse for tool calls (OpenAI format)
    size_t tc_count = 0;
...
typedef struct {
    ...
    TokenUsage* usage;
    ProviderError error;
} OpenRouterStreamBridge;

// Bridge callback for stream chunks
static void openrouter_stream_chunk_bridge(const char* chunk, void* ctx) {
...
static void openrouter_stream_error_bridge(ProviderError error, const char* message, void* ctx) {
    OpenRouterStreamBridge* bridge = (OpenRouterStreamBridge*)ctx;
    if (bridge) {
        bridge->error = error;
...
static ProviderError openrouter_stream_chat(Provider* self, const char* model, const char* system,
    ...
    // Create streaming context - uses OpenAI SSE format (compatible)
    StreamContext* stream_ctx = stream_context_create(PROVIDER_OPENROUTER);
    if (!stream_ctx) {
        free(json_body);
        return PROVIDER_ERR_NETWORK;
    }
    ...
    int result = stream_execute(stream_ctx, OPENROUTER_API_URL, json_body, api_key);
...
Provider* openrouter_provider_create(void) {
    ...
    provider->type = PROVIDER_OPENROUTER;
    provider->name = "OpenRouter";
    provider->api_key_env = "OPENROUTER_API_KEY";
    provider->base_url = OPENROUTER_API_URL;
    provider->initialized = false;

    provider->init = openrouter_init;
    provider->shutdown = openrouter_shutdown;
    provider->validate_key = openrouter_validate_key;
    provider->chat = openrouter_chat;
    provider->chat_with_tools = openrouter_chat_with_tools;
    provider->stream_chat = openrouter_stream_chat;
    provider->estimate_tokens = openrouter_estimate_tokens;
    provider->get_last_error = openrouter_get_last_error;
    provider->list_models = openrouter_list_models;
