...
static char* gemini_chat(Provider* self, const char* model, const char* system,
    ...
    const char* api_model = model ? model : "gemini-2.0-flash";
    LOG_DEBUG(LOG_CAT_API, "Gemini API call: model=%s", api_model);
    ...
    if (res == CURLE_ABORTED_BY_CALLBACK) {
        ...
        if (result) {
            telemetry_record_api_call("gemini", api_model, tokens_input, tokens_output, latency_ms);
        }
    }
