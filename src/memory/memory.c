...
int memory_generate_summary(const char* agent_name,
    ...
    if (!response) {
        ...
        snprintf(entry->summary, sizeof(entry->summary),
            "Conversation with %s (%d messages)", agent_name, message_count);
    ...
    if (json) {
        ...
        if (item && cJSON_IsNumber(item)) {
            ...
            if (entry->importance > 5) entry->importance = 5;
        }
...
static int compare_memories_by_time(const void* a, const void* b) {
    ...
    if (mb->timestamp > ma->timestamp) return 1;
    if (mb->timestamp < ma->timestamp) return -1;
    return 0;
}

int memory_load_recent(int max_entries, MemorySearchResult* result) {
    if (!result || max_entries <= 0) return -1;
...
int memory_search(const char* query, int max_results, MemorySearchResult* result) {
    if (!query || !result || max_results <= 0) return -1;
    ...
    while ((entry = readdir(d)) != NULL && result->count < (size_t)max_results) {
        ...
        MemoryEntry mem;
        if (load_memory_entry(filepath, &mem) != 0) continue;

        // Search in summary, topics, agent_name
        char search_text[4096];
        snprintf(search_text, sizeof(search_text), "%s %s %s %s",
            mem.summary, mem.topics, mem.agent_name, mem.decisions);
...
int memory_load_by_agent(const char* agent_name, int max_entries, MemorySearchResult* result) {
    if (!agent_name || !result || max_entries <= 0) return -1;
