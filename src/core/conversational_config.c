...
static void append_to_history(ConversationState* state, const char* speaker, const char* message) {
    ...
    if (remaining < 100) return;  // Safety margin

    int written = snprintf(
        state->history + state->history_len,
        remaining,
        "%s: %s\n",
        speaker, message
    );
...
static void check_fields_in_history(
    ...
    for (size_t i = 0; i < config->required_count && i < CC_MAX_REQUIRED_FIELDS; i++) {
        if (state->fields_gathered[i]) continue;
...
static char* extract_json_from_conversation(
    const ConversationalConfig* config,
    const char* history
) {
    if (!llm_is_available()) {
    ...
    snprintf(prompt, sizeof(prompt),
        ...
        history,
        config->extraction_schema ? config->extraction_schema : "{}"
    );

    // Call LLM for extraction
    TokenUsage usage = {0};
    char* response = llm_chat(
        config->extraction_prompt ? config->extraction_prompt :
...
static ConversationalResult run_fallback_form(
    const ConversationalConfig* config,
    FILE* input,
    FILE* output
) {
    ...
    for (size_t i = 0; i < config->required_count && i < CC_MAX_REQUIRED_FIELDS; i++) {
        const char* field = config->required_fields[i];
        const char* prompt = (config->fallback_prompts[i])
            ? config->fallback_prompts[i]
            : field;
...
static ConversationalResult run_conversation(
    const ConversationalConfig* config,
    FILE* input,
    FILE* output
) {
    ...
    while (state.turn_count < max_turns) {
        ...
        if (is_exit_command(user_input)) {
            print_persona(output, persona, color,
                "Va bene, ci vediamo! Torna quando vuoi.");
            result.error = strdup("User cancelled");
            return result;
        }
        ...
        snprintf(prompt, sizeof(prompt),
            ...
            state.fields_gathered_count, config->required_count,
        ...
        // Get LLM response
        TokenUsage usage = {0};
        char* llm_response = llm_chat(
            config->persona_prompt ? config->persona_prompt :
                "You are a friendly assistant gathering information through natural conversation.",
            prompt,
            &usage
        );
...
bool conversational_config_validate(
    ...
    if (!json || !required_fields) return false;
...
char* conversational_config_build_extraction_prompt(
    ...
    if (!schema || !buffer || buffer_size < 256) return NULL;
...
ConversationalConfig conversational_config_preset_project(void) {
    ...
    config.persona_prompt =
        "You are Convergio, helping set up a new software project.\n"
...
ConversationalConfig conversational_config_preset_preferences(void) {
    ...
    config.persona_prompt =
        "You are Convergio, helping customize the user experience.\n"
