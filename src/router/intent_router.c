...
static const char* cache_lookup(const char* input, float* confidence) {
    ...
    for (size_t i = 0; i < ROUTE_CACHE_SIZE; i++) {
        if (g_route_cache[i].input_hash &&
            strcmp(g_route_cache[i].input_hash, hash_str) == 0 &&
            (now - g_route_cache[i].timestamp) < 300) {  // 5 min cache
            if (confidence) *confidence = g_route_cache[i].confidence;
...
static const char* check_switch_intent(const char* input) {
    ...
    bool is_switch = (strstr(lower, "passami") || strstr(lower, "passa mi") ||
                      strstr(lower, "fammi parlare") || strstr(lower, "voglio parlare") ||
                      strstr(lower, "mettimi con") || strstr(lower, "metti mi con") ||
                      strstr(lower, "let me talk") || strstr(lower, "switch to") ||
                      strstr(lower, "connect me") || strstr(lower, "pass me to") ||
                      strstr(lower, "talk to") || strstr(lower, "parla con") ||
                      strstr(lower, "chiamami") || strstr(lower, "chiama mi"));

    if (!is_switch) return NULL;

    // Find which agent they want to switch to
    if (strstr(lower, "ali")) return "ali";
    if (strstr(lower, "amy")) return "amy-cfo";
    if (strstr(lower, "dario")) return "dario-debugger";
    if (strstr(lower, "rex")) return "rex-code-reviewer";
    if (strstr(lower, "baccio")) return "baccio-tech-architect";
    if (strstr(lower, "otto")) return "otto-performance-optimizer";
    if (strstr(lower, "luca")) return "luca-security-expert";
    if (strstr(lower, "marco")) return "marco-devops-engineer";
    if (strstr(lower, "sara")) return "sara-ux-ui-designer";
    if (strstr(lower, "omri")) return "omri-data-scientist";
    if (strstr(lower, "sofia")) return "sofia-marketing-strategist";
    if (strstr(lower, "elena")) return "elena-legal-compliance-expert";
    if (strstr(lower, "davide")) return "davide-project-manager";
    if (strstr(lower, "anna")) return "anna-executive-assistant";
    if (strstr(lower, "jenny")) return "jenny-inclusive-accessibility-champion";
    if (strstr(lower, "thor")) return "thor-quality-assurance-guardian";
...
static const char* quick_pattern_route(const char* input) {
    ...
    if (strstr(lower, "dario") || strstr(lower, "debug")) return "dario-debugger";
    if (strstr(lower, "amy") || strstr(lower, "finanz") || strstr(lower, "cost") || strstr(lower, "budget")) return "amy-cfo";
    if (strstr(lower, "rex") || strstr(lower, "review") || strstr(lower, "rivedi")) return "rex-code-reviewer";
    if (strstr(lower, "baccio") || strstr(lower, "architettura") || strstr(lower, "architect")) return "baccio-tech-architect";
    if (strstr(lower, "otto") || strstr(lower, "performance") || strstr(lower, "ottimizz")) return "otto-performance-optimizer";
    if (strstr(lower, "luca") || strstr(lower, "security") || strstr(lower, "sicurezz")) return "luca-security-expert";
    if (strstr(lower, "marco") || strstr(lower, "deploy") || strstr(lower, "docker") || strstr(lower, "ci/cd")) return "marco-devops-engineer";
    if (strstr(lower, "sara") || strstr(lower, "design") || strstr(lower, "ux") || strstr(lower, "ui")) return "sara-ux-ui-designer";
    if (strstr(lower, "omri") || strstr(lower, "data") || strstr(lower, "ml") || strstr(lower, "statistic")) return "omri-data-scientist";
    if (strstr(lower, "sofia") || strstr(lower, "marketing") || strstr(lower, "brand")) return "sofia-marketing-strategist";
    if (strstr(lower, "elena") || strstr(lower, "legal") || strstr(lower, "gdpr") || strstr(lower, "compliance")) return "elena-legal-compliance-expert";
    if (strstr(lower, "davide") || strstr(lower, "project") || strstr(lower, "timeline")) return "davide-project-manager";
    if (strstr(lower, "anna") || strstr(lower, "remind") || strstr(lower, "task") || strstr(lower, "schedul")) return "anna-executive-assistant";

    // Bug-related keywords -> Dario
    if (strstr(lower, "bug") || strstr(lower, "error") || strstr(lower, "crash") || strstr(lower, "fix") || strstr(lower, "broken")) return "dario-debugger";
...
RouterResult intent_router_route(const char* user_input) {
    ...
    if (response) {
        ...
        if (parse_route_response(response, agent, sizeof(agent), &confidence, intent, sizeof(intent))) {
