...
extern int persistence_get_message_id_range(const char* session_id, int64_t* out_first,
                                            int64_t* out_last);
extern char* persistence_load_messages_range(const char* session_id, int64_t from_id, int64_t to_id,
                                             size_t* out_count);
...
extern int persistence_save_checkpoint(const char* session_id, int checkpoint_num,
                                       int64_t from_msg_id, int64_t to_msg_id,
                                       int messages_compressed, const char* summary,
...
size_t compaction_set_dynamic_threshold(size_t model_context_window) {
    ...
    LOG_INFO(LOG_CAT_MEMORY,
             "Dynamic compaction threshold set to %zu tokens "
             "(model context: %zu, ratio: %.0f%%)",
             g_dynamic_threshold, model_context_window, COMPACTION_THRESHOLD_RATIO * 100);
...
CompactionResult* compaction_summarize(const char* session_id, int64_t from_msg_id,
                                       int64_t to_msg_id, const char* messages_text) {
    if (!session_id || !messages_text)
        return NULL;
...
#pragma clang diagnostic pop
            ...
            summary = llm_chat_with_model(
                COMPACTION_MODEL, "You are a precise and concise summarizer.", full_prompt, &usage);
...
#undef TRUNCATE_CHUNK
    ...
    int save_result = persistence_save_checkpoint(
        session_id, checkpoint_num, from_msg_id, to_msg_id, messages_compressed, summary,
        NULL, // key_facts (could extract later)
        result->original_tokens, result->compressed_tokens, result->cost_usd);
    ...
    LOG_INFO(LOG_CAT_MEMORY,
             "Compaction complete: %zu -> %zu tokens (%.1fx compression), cost: $%.6f",
             result->original_tokens, result->compressed_tokens, result->compression_ratio,
             result->cost_usd);
...
char* compaction_build_context(const char* session_id, const char* user_input,
                               bool* out_was_compacted) {
    if (!session_id || !user_input)
        return NULL;
    if (out_was_compacted)
        *out_was_compacted = false;
    ...
    int64_t cutoff_msg_id =
        persistence_get_cutoff_message_id(session_id, COMPACTION_KEEP_RECENT_MSGS);
    ...
    size_t old_count = 0;
    char* old_messages =
        persistence_load_messages_range(session_id, first_msg_id, cutoff_msg_id, &old_count);
    ...
    if (!checkpoint_summary && old_messages) {
        CompactionResult* result =
            compaction_summarize(session_id, first_msg_id, cutoff_msg_id, old_messages);
        if (result) {
            checkpoint_summary = strdup(result->summary);
            compaction_result_free(result);
            if (out_was_compacted)
                *out_was_compacted = true;
        }
    }
    ...
    char* recent = persistence_load_conversation_context(session_id, COMPACTION_KEEP_RECENT_MSGS);
    ...
    if (recent) {
        int written =
            snprintf(context + len, ctx_size - len, "## Recent Conversation\n%s\n", recent);
        if (written > 0) {
            len += (size_t)written;
        }
        free(recent);
...
int compaction_get_stats(const char* session_id, size_t* out_total_saved, double* out_total_cost) {
