...
pub(crate) fn write_env_file(api_key: &str) -> io::Result<()> {
    let api_key = crate::security::sanitize_env_value(api_key)
...
pub(crate) fn render_config(node_name: &str, role: &str, use_tailscale: bool) -> String {
    ...
    let safe_name = crate::security::escape_toml_value(node_name);
    let safe_role = crate::security::escape_toml_value(role);
    format!(
